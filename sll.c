...
/*
 *  Type: SLLNODE
 *  Description: This is the SLLNODE struct used for the implementation
 *  of the SLL class.
 */
typedef struct node {
    void *value;
...
/*
 *  Constructor: newSLLNODE
 *  Usage: SLLNODE *n = newSLLNODE(value, next);
 *  Description: This constructor creates a new SLLNODE object with a value
 *  and initializes the node's next pointer.
 */
SLLNODE *newSLLNODE(void *value, SLLNODE *next) {
...
void addToFront(SLL *items, void *value) {
    assert(items != 0);
    items->head = newSLLNODE(value, items->head);
    if (items->size == 0) {
...
void addToBack(SLL *items, void *value) {
    ...
    if (items->size == 0) {
        ...
        items->tail->next = newSLLNODE(value, NULL);
...
void insertAtIndex(SLL *items, int index, void *value) {
    ...
    if (index == 0) {
        ...
        SLLNODE *n = newSLLNODE(value, curr->next);
        curr->next = n;
        items->size++;
    }
}
